...
void Camellia_Feistel_t(const u32 *s0, const u32 *s1, u32 *s2, u32 *s3,const u32 *key){
    u32 t0, t1, t2, t3;
        t0  = *s0 ^ (key)[0];
        t3  = SBOX4_4404[t0&0xff];
        t3 ^= SBOX3_3033[(t0 >> 8)&0xff];
        t3 ^= SBOX2_0222[(t0 >> 16)&0xff];
        t3 ^= SBOX1_1110[(t0 >> 24)];
        t1  = *s1 ^ (key)[1];
        t2  = SBOX1_1110[t1&0xff];
        t2 ^= SBOX4_4404[(t1 >> 8)&0xff];
        t2 ^= SBOX3_3033[(t1 >> 16)&0xff];
        t2 ^= SBOX2_0222[(t1 >> 24)];
        t2 ^= t3;
        *s2 ^= t2; 
        *s3 ^= t2;
...
int Camellia_Ekeygen(int keyBitLength, const u8 *rawKey, KEY_TABLE_TYPE k)
{
    ...
    Camellia_Feistel_t(&s0, &s1, &s2, &s3, SIGMA + 0);
    Camellia_Feistel_t(&s2, &s3, &s0, &s1, SIGMA + 2);

    s0 ^= k[0], s1 ^= k[1], s2 ^= k[2], s3 ^= k[3];
    Camellia_Feistel_t(&s0, &s1, &s2, &s3, SIGMA + 4);
    Camellia_Feistel_t(&s2, &s3, &s0, &s1, SIGMA + 6);
    ...
    if (keyBitLength == 128) {
        ...
        Camellia_Feistel_t(&s0, &s1, &s2, &s3, (SIGMA + 8));
        Camellia_Feistel_t(&s2, &s3, &s0, &s1, (SIGMA + 10));
                                  ...
                                  u8 ciphertext[])
{
    ...
    while (1) {
        ...
        Camellia_Feistel_t(&s0, &s1, &s2, &s3, k + 0);
        Camellia_Feistel_t(&s2, &s3, &s0, &s1, k + 2);
        Camellia_Feistel_t(&s0, &s1, &s2, &s3, k + 4);
        Camellia_Feistel_t(&s2, &s3, &s0, &s1, k + 6);
        Camellia_Feistel_t(&s0, &s1, &s2, &s3, k + 8);
        Camellia_Feistel_t(&s2, &s3, &s0, &s1, k + 10);
        k += 12;
                                  ...
                                  u8 plaintext[])
{
    ...
    while (1) {
        /* Camellia makes 6 Feistel rounds */
        k -= 12;
        Camellia_Feistel_t(&s0, &s1, &s2, &s3, k + 10);
        Camellia_Feistel_t(&s2, &s3, &s0, &s1, k + 8);
        Camellia_Feistel_t(&s0, &s1, &s2, &s3, k + 6);
        Camellia_Feistel_t(&s2, &s3, &s0, &s1, k + 4);
        Camellia_Feistel_t(&s0, &s1, &s2, &s3, k + 2);
        Camellia_Feistel_t(&s2, &s3, &s0, &s1, k + 0);
...
void Camellia_decrypt_128(const u8 *in, const u8 *key, u8 *out){
    KEY_TABLE_TYPE subkey;
    Camellia_Ekeygen(128, key, subkey);
    Camellia_DecryptBlock(128, in, subkey, out);
}

void Camellia_encrypt_192(const u8 *in, const u8 *key, u8 *out){
    KEY_TABLE_TYPE subkey;
    Camellia_Ekeygen(192, key, subkey);
    Camellia_EncryptBlock(192, in, subkey, out);
}

void Camellia_decrypt_192(const u8 *in, const u8 *key, u8 *out){
    KEY_TABLE_TYPE subkey;
    Camellia_Ekeygen(192, key, subkey);
    Camellia_DecryptBlock(192, in, subkey, out);
}

void Camellia_encrypt_256(const u8 *in, const u8 *key, u8 *out){
    KEY_TABLE_TYPE subkey;
    Camellia_Ekeygen(256, key, subkey);
    Camellia_EncryptBlock(256, in, subkey, out);
}

void Camellia_decrypt_256(const u8 *in, const u8 *key, u8 *out){
    KEY_TABLE_TYPE subkey;
    Camellia_Ekeygen(256, key, subkey);
    Camellia_DecryptBlock(256, in, subkey, out);
}

void test(){
    u8 in[] = {0x01, 0x23, 0x45,0x67, 0x89, 0xab, 0xcd, 0xef, 0xfe, 0xdc, 0xba, 0x98, 0x76, 0x54, 0x32, 0x10};
    u8 key1[] = {0x01, 0x23, 0x45,0x67, 0x89, 0xab, 0xcd, 0xef, 0xfe, 0xdc, 0xba, 0x98, 0x76, 0x54, 0x32, 0x10};
    u8 key2[] = {0x01, 0x23, 0x45,0x67, 0x89, 0xab, 0xcd, 0xef, 0xfe, 0xdc, 0xba, 0x98, 0x76, 0x54, 0x32, 0x10, 0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77};
    u8 key3[] = {0x01, 0x23, 0x45,0x67, 0x89, 0xab, 0xcd, 0xef, 0xfe, 0xdc, 0xba, 0x98, 0x76, 0x54, 0x32, 0x10, 0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x99, 0xaa, 0xbb, 0xcc, 0xdd, 0xee, 0xff}; 
    u8 out1[16], out2[16];
    
    // encryption and decencryption with 128-bit key test
    printf("test enc&dec_128 function:\n");
    Camellia_encrypt_128(in, key1, out1);
    ...
    Camellia_decrypt_128(out1, key1, out2);
    for (int i = 0; i < 16; ++i){
        if (out2[i] > 9)
            printf("%x", out2[i]);
        else
    ...
    // encryption and decencryption with 192-bit key test
    printf("test enc&dec_192 function:\n");
    Camellia_encrypt_192(in, key2, out1);
    // b499341b3e996f84ee5cee7d79b9b9
    for (int i = 0; i < 16; ++i){
        if (out1[i] > 9)
            printf("%x", out1[i]);
        else
    ...
    printf("\n");
    Camellia_decrypt_192(out1, key2, out2);
    for (int i = 0; i < 16; ++i){
        if (out2[i] > 9)
            printf("%x", out2[i]);
        else
    ...
    // encryption and decencryption with 256-bit key test
    printf("test enc&dec_256 function:\n");
    Camellia_encrypt_256(in, key3, out1);
    ...
    printf("\n");
    Camellia_decrypt_256(out1, key3, out2);
    for (int i = 0; i < 16; ++i){
        if (out2[i] > 9)
            printf("%x", out2[i]);
        else
